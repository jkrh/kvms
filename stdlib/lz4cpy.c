...
╚─────────────────────────────────────────────────────────────────────────────*/
#include "cosmo/bits.h"
#include "cosmo/pushpop.h"
#include "cosmo/repmovsb.h"
#include "cosmo/kompressor.h"
 ...
 */
void *lz4cpy(void *dest, void *blockdata, size_t blocksize) {
  ...
  for (op = dest, ip = blockdata, ipe = ip + blocksize;;) {
    ...
    repmovsb((void **)&op, (void **)&ip, length);
    if (ip >= ipe) break;
    offset = READ16LE(ip);
    ...
    match = op - offset;
    repmovsb((void **)&op, (void **)&match, (matchlen += 4));
  }
  return op;
}
